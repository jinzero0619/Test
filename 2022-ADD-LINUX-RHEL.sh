...
#================================================================================
# RELEASE BingoCVM ADD, Made By KBI CORP.
# 2024 03 13 Version!
#===============================================================================
# VAR start
LANG=C
...
case $MAIN_LINUX in
	...
	Oracle)
	LV=`cat /etc/system-release | awk '{ print $5 }' | cut -c 1`
    ;;
	...
	VC=$(cat /tmp/MAINVER | grep el | cut -c3)

	if [ "$VC" -ge 7 ]
	then
			LV=$(cat /etc/redhat-release | awk '{ print $4 }' | cut -c1)
	else
			LV=$(cat /etc/redhat-release | awk '{ print $3 }' | cut -c1)
	fi
    ;;
    *)
    echo "Nothing"
...
LOG_SECTION=`echo -e "======================================================\n Status of part of the file \n======================================================"`
LOG_ALL=`echo -e "\n======================================================\n All status of the file \n======================================================"`
LOG_PERMISSION=`echo -e "======================================================\n Current file permissions \n======================================================"`
...
# Create Directory
rm -rf /tmp/2.9.9/
mkdir -p /tmp/2.9.9/result
HTE=`hostname`

if [ `ip route get 1.2.3.4 2> /dev/null | grep -ic "via"` -eq 0 ]
then
    IPA=`ip route get 1.2.3.4 2> /dev/null | xargs | awk '{ print $5 }'`
else
    IPA=`ip route get 1.2.3.4 2> /dev/null | xargs | awk '{ print $7 }'`
fi
...
for i in `seq 1 9`
do
        echo KB-AU-E-01 > /tmp/2.9.9/result/0$i.result
        echo KB-AU-E-01 > /tmp/2.9.9/result/0$i.log
        echo KB-AU-E-01 > /tmp/2.9.9/result/0$i.title
        echo KB-AU-E-01 > /tmp/2.9.9/result/0$i.msg
done

for i in `seq 10 75`
do
        echo KB-AU-E-01 > /tmp/2.9.9/result/$i.result
        echo KB-AU-E-01 > /tmp/2.9.9/result/$i.log
        echo KB-AU-E-01 > /tmp/2.9.9/result/$i.title
        echo KB-AU-E-01 > /tmp/2.9.9/result/$i.msg
done
# Only MANUAL END A
...
case $MAIN_LINUX in
	...
	if [ `cat /tmp/MAINVER | grep el | cut -c 3` -ge 7 ]
	then
		L=`cat /etc/redhat-release | awk '{ print $1 }'`
		LV=`cat /etc/redhat-release | awk '{ print $4 }' | cut -c 1-3`
	else
...
echo osn: $L >> /tmp/2.9.9/result/chkver
echo osv: $LV >> /tmp/2.9.9/result/chkver
...
cd /tmp/2.9.9/result/ && tar -cf /tmp/$IPA.tar *
rm -rf /tmp/2.9.9
# Only MANUAL END B
